...
@property (nonatomic,strong)   FNCommentDetailInfoView *detailInfoView;
...
- (void)configurePlaceTypeUI
{
    ...
    switch (_placeType) {
        ...
        case FNPlaceLeftTop:
            ...
            break;
        case FNPlaceLeftBottom:
            
            break;
        case FNPlaceRightTop:
            
            break;
        case FNPlaceRightBpttom:
            
            break;
        case FNPlaceAll:
            [self configureCommentList];
            break;
        default:
            break;
    }
...
- (void)configureCommentList
{
    ...
    FNCommentDetailInfoView *detailInfoView = [[FNCommentDetailInfoView alloc] initWithFrame:CGRectZero];
    self.detailInfoView = detailInfoView;
    [self.view addSubview:detailInfoView];
    FNFullScreenImage *image = [self safeObjectAtIndex:self.currentIndex ofArray:self.fullScreenImageArray];
    [detailInfoView setImageModel:image isAnimation:NO];
}
...
- (void)disappear
{
    ...
    if (viewcontrollers.count>1) {
        ...
        //present方式
        [self dismissViewControllerAnimated:self.isEndAnimation completion:nil];
    }
...
- (void)configureNavigation
{
    ...
    titleLabel.text = [NSString stringWithFormat:@"%ld/%ld",self.currentIndex+1,self.images.count];
    ...
    UIImage *leftImage = [UIImage imageNamedBD:@"icon_return"];
    ...
    UIImage *image = [UIImage imageNamedBD:@"icon_photo_more"];
...
- (void)updateCurrentImageView {
    ...
    currentImageView.placeHolderImage = _placeHolderImage;
...
- (void)configPageType
{
    ...
    if (self.pageType == FNPageTypeLabel){
    ...
    } else if (self.pageType == FNPageTypeControl){
        ...
        pageControl.frame = CGRectMake(kScreenWidth/2.0-100, kScreenHeight-20, 200, 20);
        pageControl.numberOfPages = self.imageUrlArray ? (self.imageUrlArray.count>1?self.imageUrlArray.count:0) : (self.images.count>1?self.images.count:0);
        pageControl.pageIndicatorTintColor = [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1];
...
- (void)updateImageView:(FNFullScreenImageView *)imageView atIndex:(NSInteger)index
{
    ...
    imageView.tag = index;
    FNFullScreenImage *imageModel = [self safeObjectAtIndex:index ofArray:self.images];
    __block FNFullImageAnimation *animation = self.animation;
    WS(weakSelf);
    imageView.hideDetailView = ^
    {
        if (weakSelf.tapType == FNTapTypeHide) {
            if (weakSelf.isDisAppearIntoMiddleOfAnimation) {
                animation.disappearIntoMiddleOfAnimation = (index != 0);
            }
            
            if (weakSelf.disMissAtIndex) {
                weakSelf.disMissAtIndex(index);
            }
            
            animation.image = imageModel.image;
            [weakSelf disappear];
        } else {
            [weakSelf hideNavgationBar:!weakSelf.navigationController.navigationBarHidden];
        }
    };
...
//    隐藏导航栏
    ...
    if (index != self.currentIndex) {
        ...
        FNFullScreenImage *image = [self safeObjectAtIndex:self.currentIndex ofArray:self.fullScreenImageArray];
        if (self.placeType == FNPlaceAll) {
            [self.detailInfoView setImageModel:image isAnimation:NO];
        }
